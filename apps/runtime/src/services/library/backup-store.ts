 ...
 *
 * The backup root is always caller-supplied. This host never invents
 * `~/.mango/library-backups` or reads hub configuration — remote environments
 * must receive an explicit path from the hub that owns retention policy.
 */
...
export interface CreateBackupStoreDepsOptions {
...
const DEFAULT_RETENTION_BYTES = 512 * 1024 * 1024;
...
/** Builds backup-store deps from an explicit root. Never reads hub config. */
export function createBackupStoreDeps(options: CreateBackupStoreDepsOptions): BackupStoreDeps {
  return {
    fs: options.fs ?? nodeBackupStoreFs,
    backupDir: asGetter(options.backupRoot),
    retentionCount: asGetter(options.retentionCount ?? DEFAULT_RETENTION_COUNT),
    retentionBytes: asGetter(options.retentionBytes ?? DEFAULT_RETENTION_BYTES),
    now: options.now ?? (() => new Date()),
    randomSuffix: options.randomSuffix ?? (() => randomBytes(8).toString('hex')),
  };
}
...
export function createBackupId(deps: BackupStoreDeps): string {
 ...
 */
export async function restoreBackupEntry(entry: BackupEntry, deps: BackupStoreDeps): Promise<void> {
  if (!entry.backupPath) {
...
export async function discardBackupSet(backupId: string, deps: BackupStoreDeps): Promise<void> {
 ...
 */
export async function purgeBackupSet(backupId: string, deps: BackupStoreDeps): Promise<boolean> {
 ...
 * misconfigured backup root pointing at a shared directory must never make
 * retention delete unrelated data, and a set that vanished mid-prune is simply
 * skipped.
 */
