/**
 * Every child process the runtime spawns must hide its console window on
 * Windows — the `pwsh` shell slot is the most visible one on a native-Windows
 * runtime, but the rule applies to every spawn site. Twin of
 * `apps/api/tests/unit/lib/hidden-window.test.ts`; see that file for the
 * reasoning and the precedent it follows.
 */
...
describe('every child-process spawn hides its console window on Windows', () => {
  const files = sourceFilesUnder(RUNTIME_SRC);
