/**
 * Matching branches to their pull requests, and the one question the match
 * exists to answer: is deleting this branch safe.
 */
...
describe('annotateBranchesWithPrs', () => {
  ...
  it('marks only a merged pull request as safe to delete', () => {
    ...
    expect(annotations.get('merged-branch')?.safeToDelete).toBe(true);
    expect(annotations.get('open-branch')?.safeToDelete).toBe(false);
    // Closed-without-merging is the case that matters: the work is *not* in the
    // base branch, so deleting the branch loses it.
    expect(annotations.get('closed-branch')?.safeToDelete).toBe(false);
  });
  ...
  it('keeps the highest-numbered pull request when a branch has several', () => {
    ...
    expect(annotations.get('feat/x')?.safeToDelete).toBe(false);
  });
