/**
 * Skills settings query keys and options.
 */
...
export function skillSettingsListQueryOptions() {
  return queryOptions({
    ...
    staleTime: 30_000,
    queryFn: async () => {
      const { data, error } = await client.api.settings.skills.get();
      if (error)
        throw new Error(extractApiError(error.value, en.settings.skills.skillsList.loadError));
