...
const USER_ID = 'skills-prompt-section-test-user';
...
describe('appendSkillsPromptSection', () => {
  ...
  it('leaves the prompt unchanged when the skill tool is not allowed', async () => {
    installSkill();
    expect(
      await appendSkillsPromptSection('Base prompt.', new Set(['read_file']), getDb(), USER_ID)
    ).toBe('Base prompt.');
  });
  ...
  it('leaves the prompt unchanged when no usable skills exist', async () => {
    ...
    resetSkillsCache();
    expect(
      await appendSkillsPromptSection('Base prompt.', new Set([SKILL_TOOL_NAME]), getDb(), USER_ID)
    ).toBe('Base prompt.');
    expect(
      await appendSkillsPromptSection(undefined, new Set([SKILL_TOOL_NAME]), getDb(), USER_ID)
    ).toBeUndefined();
  });
});
