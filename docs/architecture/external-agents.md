...
over rather than resuming. Graceful interruption on Windows stays tracked separately and is not
...
| steer: forwards, passes a rejection through, answers not-supported, refuses a closed session   | `steering_is_answered_not_thrown_when_it_cannot_land`, `a_duplicate_steer_waiting_on_one_that_fails_in_transit_receives_its_failure`                               | covered                                           |
| bounds concurrent sessions and recovers capacity after close                                   | `the_session_cap_counts_opening_sessions`                                                                                                                          | covered                                           |
...
| turns a mid-stream adapter crash into a bounded error and cancellation                         | `error_carries_code_message_vendor_code_request_id_and_retryable`; the SDK owns the crash-to-error mapping                                                         | covered, SDK half in the SDK                      |
...
| enforces the hard turn deadline even while a stream remains active                             | `a_turn_past_its_hard_deadline_ends_with_its_own_error_and_frees_the_session`                                                                                      | covered                                           |
