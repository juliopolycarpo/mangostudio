...
- **Main-agent verification**: the orchestrator validates that the delegation result satisfies the response contract; if it is missing or invalid, it retries once with stricter output requirements and then falls back to a standardized failure result.
