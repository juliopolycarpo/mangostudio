...
`bun run build --binary` compila a API em binários específicos por plataforma via `bun build --compile`. Os assets do frontend são embarcados como arquivos sidecar. Há suporte a 8 plataformas (linux/windows/darwin × x64/arm64 + glibc/musl). O banco de dados usa `~/.mangostudio/database.sqlite` por padrão.
