...
7. Configure Trusted Publishing no npm para `mangostudio` e cada pacote `@mangostudio/cli-*` (**Settings -> Trusted Publisher**): repositório `juliopolycarpo/mangostudio`, workflow `release.yml`, environment `release`, ação `npm publish`. O npm permite até 10 trusted publishers por pacote, então o canary pode ter a própria configuração em vez de compartilhar esta — o workflow a registrar é `ci.yml`, não `canary.yml`. Enquanto não tiver uma, o canary continua com o `NPM_TOKEN` do repositório; não ative “disallow tokens” no npm enquanto ele ainda precisar desse token.
