...
| `CARGO_REGISTRY_TOKEN`      | `cargo-publish`                          | Token de publicação no crates.io para o crate `mangostudio`                                               |
| *(built-in `GITHUB_TOKEN`)* | `github-release`, `docker`, attestations | Sem setup extra — o workflow concede `packages: write` para GHCR                                          |
...
3. Reserve o nome do crate `mangostudio` no [crates.io](https://crates.io) e gere um token de API.
4. Adicione os três repo secrets (`NPM_TOKEN`, `DIST_REPOS_TOKEN`, `CARGO_REGISTRY_TOKEN`) neste repositório.
5. Após o primeiro push no GHCR, defina a visibilidade do pacote `ghcr.io/juliopolycarpo/mangostudio` como **public** nas configurações de pacotes do GitHub.
6. Garanta que a proteção de branch permita ao bot de release fazer push de `CHANGELOG.md` em `main` (`contents: write` no workflow).
...
O workflow executa nove jobs: `build`, `github-release`, `docker`, `npm-publish`,
`homebrew`, `scoop`, `cargo-publish`, `verify-release` e `update-changelog`. Veja
a [versão em inglês](../../reference/releasing.md#cutting-a-release) para a
tabela completa de jobs e detalhes por canal (npm, Docker, Homebrew, Scoop,
crates.io).
