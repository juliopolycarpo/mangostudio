...
| `env install <recipe> [--environment <id>] [--version <spec>]` | Executa uma receita de instalação nesta máquina ou em uma pareada.            |
...
para rodar em uma máquina pareada, e `--version` para receitas que aceitam uma
versão do Node. Saem com `0` em sucesso, `1` se a execução terminar em
qualquer status diferente de `succeeded`, e `2` quando a receita nunca chega a
rodar (bloqueada, não suportada, faltando um pré-requisito, ou somente-cópia).
