...
That boundary is now reported rather than left implicit. A tool declares `uncheckpointedWriteSource` in its settings metadata (`buildShellTool` sets `'shell'`; every MCP call counts as `'mcp'`, because the hub cannot know whether a foreign tool writes), and the generation layer records it against the turn's message in `message_uncheckpointed_sources` before the call runs. Both `GET /api/chats/:id/checkpoints` and the revert response carry `uncheckpointedSources`, and the confirmation dialog and result toast name the classes the revert will not undo. Adding a tool that writes outside the manifest means setting that field, not extending a list of tool names. Extending *coverage* instead of reporting it — a post-hoc workdir diff for shell, declared writes for MCP — is a separate design question and is not attempted here.
