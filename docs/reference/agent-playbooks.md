...
- `apps/runtime/src/services/external-agents/codex/protocol/` (**generated** — the vendor's own
  contract, regenerated by `bun run vendor:codex-protocol`, never edited by hand)
...
The pinned contract is regenerated with `bun run vendor:codex-protocol` and drift-checked in CI by
`.github/workflows/vendor-drift.yml`. The pin is a **package invocation**, not a global binary, so
it reproduces on a runner with no Codex installed — and it is unrelated to how a user installed
theirs, since the adapter resolves the executable through the runtime scanner and gates on
`codex --version`.
