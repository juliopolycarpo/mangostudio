...
├── vendor/           Regenerate + drift-check committed vendor contracts (bun run vendor:codex-protocol)
...
Some third-party contracts are generated by the vendor and committed verbatim, so a version bump
shows up as a reviewable diff instead of a silent behaviour change.

| Script              | Purpose                                                         |
| ------------------- | --------------------------------------------------------------- |
| `codex-protocol.ts` | Regenerate or drift-check the Codex `app-server` TypeScript API |

```bash
bun run vendor:codex-protocol           # regenerate in place
bun run vendor:codex-protocol --check   # fail on any difference (what CI runs)
```

The generator is invoked as a **pinned package** (`bunx @openai/codex@<version>`), read from
...
`protocol/` is formatted by Biome — formatting vendor output would make every regeneration report a
diff that is ours rather than theirs.
