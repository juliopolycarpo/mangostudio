...
describe('normalizeSchema', () => {
  ...
  it('inlines a $ref hidden behind a schemars null alternative', () => {
    // Annotated rather than inferred: the branches have disjoint keys, so TypeScript
    // widens them to a union carrying `type?: undefined`, which no index signature
    // of `Json` accepts.
    const schema: Json = { anyOf: [{ $ref: '#/$defs/id' }, { type: 'null' }] };
