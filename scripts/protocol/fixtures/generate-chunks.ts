 ...
 *
 * @example
 * bun ./scripts/protocol/fixtures/generate-chunks.ts          # rewrite the file
 * bun ./scripts/protocol/fixtures/generate-chunks.ts --check  # exit 1 when stale
 */
...
const document = {
  $comment:
    'WebSocket chunk-framing corpus, generated by scripts/protocol/fixtures/generate-chunks.ts. messages are base64 binary WebSocket messages fed in order; expected is the reassembled frame.',
  cases: build(),
};
...
if (process.argv.includes('--check')) {
  ...
  if (current !== text) {
    console.error(
      'spec/fixtures/1/chunks.json is stale; run bun ./scripts/protocol/fixtures/generate-chunks.ts'
    );
    process.exit(1);
  }
