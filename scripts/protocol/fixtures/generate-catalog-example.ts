 ...
 *
 * @example
 * bun ./scripts/protocol/fixtures/generate-catalog-example.ts          # rewrite the file
 * bun ./scripts/protocol/fixtures/generate-catalog-example.ts --check  # exit 1 when stale
 */
...
if (process.argv.includes('--check')) {
  ...
  if (current !== text) {
    console.error(
      'spec/fixtures/1/catalog-example.json is stale; run bun ./scripts/protocol/fixtures/generate-catalog-example.ts'
    );
    process.exit(1);
  }
