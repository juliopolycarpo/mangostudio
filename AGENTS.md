...
- Changelog or release: `cliff.toml`, `scripts/changelog.ts`, `scripts/lib/changelog.ts`, `scripts/lib/release-version.ts`, `scripts/check-versions.ts`, `scripts/release/pack-npm.ts`, `packages/cli/`, `packages/cargo-shim/`, `.github/workflows/{pr-qa-gate,release,cargo-shim}.yml`, `docs/reference/releasing.md`
