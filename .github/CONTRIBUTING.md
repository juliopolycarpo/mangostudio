...
- **CodeQL** (`.github/workflows/codeql.yml`) uses GitHub code scanning advanced setup for JavaScript/TypeScript and Rust, no-build extraction, and the `security-extended` query suite. Review the `CodeQL / Analyze (javascript-typescript)` and `CodeQL / Analyze (rust)` workflow checks and the follow-up `Code scanning results / CodeQL` check. New high, critical, or error-level alerts must be fixed or intentionally triaged before merge.
