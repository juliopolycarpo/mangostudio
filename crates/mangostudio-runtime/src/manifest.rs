...
//! `crate::transport::build_host` (crate-private) implements a subset of the
//! catalog (`runtime.health`, `workspace.*`, `probing.*`, `fs.*`,
//! `terminal.*`), so the same gate keeps every feature backed by a method
