...
- `cargo binstall` prebuilt strategies are intentionally disabled: the app needs its `public/`
  sidecar next to the binary, and binstall's binary-only extraction would drop it. binstall falls
  back to compiling this launcher, which installs the complete archive instead.
